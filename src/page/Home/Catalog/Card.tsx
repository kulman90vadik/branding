...
const Card: React.FC<CollectionItemProp> = ({ item }) => {
  const dispatch = useDispatch();

  const clickToCard = (obj: CollectionItem) => {
    dispatch(addToCart(obj));
    dispatch(onChengeBtn(obj));
    dispatch(onChengeBtnBasket(obj));
  };
  const clickToFavorites = (obj: CollectionItem) => {
  ...
  return (
    <li className="card">
      <div className="card__photo">
        <svg
          onClick={() => clickToFavorites(item)}
          className={`card__like ${
            item.activeLike ? "card__like--active" : ""
          }`}
          viewBox="0 0 25 25"
          fill="none"
      ...
      <div className="card__block">
        ...
        <button
          className={`card__btn btn-reset ${
            item.activeBtn ? "card__btn--green" : "card__btn--yellow"
          }`}
          type="button"
          onClick={() => clickToCard(item)}
        >
          Add to Cart
        </button>
      </div>
